---
title: 재고 및 소스
description: ' [!DNL Commerce] 상점에서 판매 가능한 수량이 총실사와 일치하도록 [!DNL Inventory Management] 에서 주식과 원본의 관계를 알아봅니다.'
...
창고 위치, 제품 또는 서비스 유형 또는 판매 채널에 관계없이 재고를 관리합니다. 발주를 이행하고 여러 위치의 제품을 출하하여 균형 잡힌 재고, 운송 비용 등에 중점을 두고 주문을 완료합니다.

이러한 설명에는 미국과 유럽에서 여러 선적 위치와 웹 사이트를 보유한 자전거 회사의 제품, 소스 및 재고가 포함됩니다.

## 소스

[source](sources-manage.md)은(는) 주문 이행을 위해 제품 인벤토리를 관리하고 배송하는 실제 위치이거나 서비스를 사용할 수 있는 실제 위치입니다. [!DNL Commerce]은(는) 재고당 수량 및 판매 가능 수량을 사용하고 관리되는 제품 및 주문에 대해 재고 금액을 자동으로 관리합니다. 소스가 하나인 경우 _단일 소스_ 모드로 간주됩니다. 소스가 여러 개인 경우 _다중 소스_ 모드로 간주됩니다.
...
판매 채널은 웹 사이트, 스토어 보기, B2B 고객 그룹 등을 포함하여 인벤토리를 판매하는 개체를 나타냅니다. 판매 채널은 하나의 Stock에만 연결할 수 있습니다. 각 판매 채널에는 단일 주식만 할당할 수 있으며 단일 주식은 여러 웹 사이트에 할당할 수 있습니다. 재고를 통해 주문을 배송할 때 사용되는 소스의 우선 순위를 [Source 선택 알고리즘](selection-reservations.md)을 통해 수정할 수 있습니다.
