...
## 예약된 가격 규칙 업데이트의 작동 방식

- 모든 예약된 업데이트가 연속적으로 적용됩니다. 즉, 모든 엔티티는 한 번에 하나의 예약된 업데이트만 가질 수 있습니다.

- 모든 예약된 업데이트는 해당 시간대 내의 모든 스토어 보기에 적용됩니다. 따라서 엔티티는 서로 다른 스토어 보기에 대해 동시에 서로 다른 예약된 업데이트를 가질 수 없습니다. 현재 예약된 업데이트의 영향을 받지 않는 모든 스토어 뷰 내의 모든 엔티티 속성 값은 이전 예약된 업데이트가 아닌 기본값에서 가져옵니다.
...
종료 일자 없이 활성 가격 규칙이 생성된 경우 특정 시점에 종료해야 하는 경우 기존 예약된 업데이트를 편집하여 종료 일자를 추가할 수 없습니다. 대신 규칙의 상태를 `Inactive`(으)로 변경하는 예약된 업데이트를 새로 만들어야 합니다. 이 새 업데이트의 시작 날짜를 판매가 종료될 날짜 및 시간으로 설정합니다.
...
1. **[!UICONTROL Rule Information]** 섹션까지 아래로 스크롤하고 필요에 따라 규칙을 변경합니다.
   ...
   >[!NOTE]
   >
   >규칙 정보 매개 변수를 업데이트할 때마다 _[!UICONTROL Status]_이(가) 올바르게 설정되어 있는지 확인하십시오. 변경 사항으로 인해 활성화된 규칙이 적용되도록 하려면 상태를 `Active`(으)로 설정하십시오.
...
1. 일정으로 돌아가려면 미리 보기 페이지의 헤더에서 **[!UICONTROL Calendar]**&#x200B;을(를) 클릭합니다.
   ...
   **미리 보기에 대한 링크 공유**
   ...
   저장소 미리 보기에 대한 링크를 다른 관리 사용자와 공유하려면 **[!UICONTROL Share]**&#x200B;을(를) 클릭하십시오. 링크를 클립보드에 복사한 다음 이메일 메시지 본문에 붙여넣습니다.

   >[!NOTE]
   >
   >[역할에 관리자 사용자 계정을 관리할 수 있는 액세스 권한](../systems/permissions-user-roles.md)이 있는 경우 미리 보기 링크를 공유할 수 있도록 관리자 권한으로 기존 사용자 계정을 만들거나 업데이트할 수 있습니다.

   **미리 보기 범위 변경**

   다른 저장소 보기에 대해 예약된 변경 내용을 보려면 미리 보기 페이지의 헤더에서 **[!UICONTROL Scope]**&#x200B;을(를) 클릭하십시오. 미리 보려는 웹 사이트, 스토어 또는 스토어 보기를 선택합니다.

1. 필요한 경우 일정으로 돌아가서 **[!UICONTROL View/Edit]** 열의 _[!UICONTROL Action]_을(를) 클릭하여 다른 예약된 업데이트를 엽니다.
