...
소스가 기존 URL이 아닌 구매자가 입력한 쿼리인 경우 검색어 리디렉션을 사용합니다. 자세한 지침은 [검색어 리디렉션 및 상점 경로 지정](search-term-redirects.md)을 참조하세요.
...
여러 URL로 액세스할 수 있는 단일 페이지가 있거나 유사한 콘텐츠를 가진 서로 다른 페이지가 있는 경우 Google은 이러한 페이지를 동일한 페이지의 중복 버전으로 봅니다. Google은 하나의 URL을 표준 버전 및 크롤링으로 선택하며, 다른 모든 URL은 중복 URL로 간주되고 크롤링은 덜 자주 사용됩니다.

Google에 어떤 URL이 정식 URL인지 명시적으로 알리지 않는 경우, 자동으로 선택되거나 두 URL을 모두 동일한 가중치로 간주할 수 있습니다. 이는 원치 않는 예산 및 낮은 분산 백링크를 비효과적 인 크롤링 행동으로 이어질 수 있습니다.
