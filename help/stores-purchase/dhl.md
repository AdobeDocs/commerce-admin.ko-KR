...
1. DHL REST API를 사용하는 경우 **[!UICONTROL DHL Type]**&#x200B;을(를) `DHL REST`(으)로 설정하십시오.

   DHL XML API를 사용하는 경우 **[!UICONTROL DHL Type]**&#x200B;을(를) `DHL XML`(으)로 설정하십시오.

   >[!NOTE]
   >
   >DHL REST API는 DHL과 통합하기 위해 선호되는 방법입니다. XML API는 더 이상 사용되지 않으며 이후 릴리스에서 제거될 수 있습니다.

1. DHL에서 제공한 자격 증명을 사용하여 다음 필드를 작성합니다.

DHL REST API를 사용하는 경우 다음 자격 증명을 제공해야 합니다.

    - **[!UICONTROL API KEY]**
    - **[!UICONTROL API SECRET]**

DHL XML API를 사용하는 경우 다음 자격 증명을 제공해야 합니다.
...
1. 요구 사항에 따라 처리 요금 옵션을 구성합니다.
   ...
   - **[!UICONTROL Handling Fee]**&#x200B;에 대해 금액을 계산하기 위해 선택한 방법에 따라 청구할 금액을 입력합니다.
     ...
     예를 들어, 요금이 고정 요금을 기준으로 하는 경우 `4.90`과(와) 같이 그 금액을 소수점으로 입력하십시오. 단, 취급 수수료가 주문의 백분율을 기준으로 하는 경우 백분율로 금액을 입력합니다. 예를 들어 주문의 6%를 부과하려면 값을 `6`(으)로 입력하십시오.
   ...
   >[!NOTE]
   >
   >차원을 지정하지 않으면 기본적으로 각각 최소값 3이 됩니다.
...
1. DHL에서 **[!UICONTROL sandbox mode]** 옵션을 제공합니다. 샌드박스 모드를 사용하는 경우 **[!UICONTROL sandbox mode]**&#x200B;을(를) `Yes`(으)로 설정하십시오.
라이브 모드를 사용하는 경우 **[!UICONTROL sandbox mode]**&#x200B;을(를) `No`(으)로 설정하십시오.

   >[!NOTE]
   >
   >샌드박스 모드는 테스트 목적으로만 사용됩니다. 라이브 스토어에 영향을 주지 않고 DHL과의 통합을 테스트할 수 있습니다.
