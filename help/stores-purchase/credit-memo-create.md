...
메모가 있는 모든 메모 [진행 중 상태](order-status.md) 미불 환불 기한이 있습니다.
...
다음을 참조하십시오 [송장 만들기](invoices.md#create-an-invoice) 추가 정보.

## 결제 작업 설정

신용 카드로 지불한 주문에 대한 환불 워크플로우는 [결제 조치 설정](../configuration-reference/sales/payment-methods.md#payment-actions) 사용 가능한 각 결제 방법에 대한 구성에서 확인할 수 있습니다. 거래가 정산될 때까지 환불을 할 수 없습니다.

![결제 조치 설정](./assets/payment-action-setting.png){width="600" zoomable="yes"}

- 구성된 결제 방법에 대한 결제 조치가 로 설정된 경우 `Authorize`(이)에서, 대변 메모를 작성하려면 먼저 책임자로부터 송장을 생성해야 합니다.
- 구성된 결제 방법에 대한 결제 조치가 로 설정된 경우 `Authorize and Capture`, 송장이 결제 프로세서에 의해 이미 생성되었지만, 거래가 결제될 때까지 자금을 사용할 수 없습니다. 이 짧은 대기 기간은 많은 결제 처리자가 보안 수단으로 권장하며 일반적으로 자동으로 처리할 수 있습니다. 또한 거래는 결제 프로세서를 통해 판매자 계정에서 수동으로 정산할 수 있습니다.
- ![Adobe Commerce](../assets/adobe-logo.svg) (Adobe Commerce만 해당) 선물 옵션이 포함된 주문에 대한 대변 메모를 생성하면 선물 포장 및/또는 인쇄된 카드에 대한 환불이 대변 메모의 환불 합계 섹션에 표시됩니다. 이러한 비용을 환불할 금액에서 제외하려면 금액을 조정 수수료로 입력합니다. 동일한 주문에 대해 여러 개의 대변 메모가 발행된 경우, 선물 옵션에 대한 환불이 첫 번째 대변 메모에만 나타납니다.

## 대변 메모 만들기

다음에 대해 발행할 환불 유형을 결정합니다. [신용 구매](#issue-a-refund-for-a-credit-purchase) 또는 [수표 또는 우편환](#issue-an-offline-refund-for-check-or-money-order)- 대변 메모를 생성하고 환불을 발행합니다.
...
1. 그리드에서 순서를 찾은 다음 **[!UICONTROL View]**.

1. 다음과 같은 경우 _[!UICONTROL Credit Memo]_단추가 단추 모음에 표시되면 다음 중 하나를 수행합니다.

   - 을(를) 발행하려면 `offline` 환불하려면 #6단계로 이동하십시오.
   - 을(를) 발행하려면 `online` 환불하려면 #4단계를 계속 진행하십시오.

   다음을 참조하십시오 [대변 메모](credit-memos.md) 오프라인 및 온라인 환불에 대한 자세한 내용은 를 참조하십시오.

1. 클릭 **[!UICONTROL Invoices]** 왼쪽 패널에서

1. 그리드에서 청구서를 찾아 **[!UICONTROL View]**.

   ![인보이스 그리드](./assets/order-invoices-grid.png){width="700" zoomable="yes"}

1. 아래로 스크롤하여 **[!UICONTROL Invoice Totals]** 송장의 섹션에서 송장이 다음으로 설정되어 있는지 확인합니다. `Capture Online`, 및 클릭 **[!UICONTROL Submit Invoice]**.

   ![온라인 캡처](./assets/order-invoice-capture-online.png){width="600" zoomable="yes"}

   해당 옵션을 사용할 수 없는 경우 송장이 이미 생성되었습니다. 다음 단계로 진행합니다.

1. 청구서 상단의 단추 모음에서 를 클릭합니다. **[!UICONTROL Credit Memo]**.

1. 에서 정보를 확인합니다. **[!UICONTROL Items to Refund]** 섹션을 참조하고 해당하는 경우 다음을 수행합니다.

   - 제품을 재고로 반품하려면 **[!UICONTROL Return to Stock]** 확인란.

     다음과 같은 경우 제품이 자동으로 재고로 반환됩니다. _제품 스톡 옵션_ 이(가) (으)로 설정됨 `Automatically Return Credit Memo Item to Stock`. 포함 [Inventory management 활성화됨](../inventory-management/enable.md)즉, 품목이 납품을 보낸 출처로 돌아갑니다.

   - 업데이트 **[!UICONTROL Qty to Refund]**, 및 클릭 **[!UICONTROL Update Qty's]**.

     ![환불할 항목](./assets/invoice-credit-memo-items-to-refund.png){width="600" zoomable="yes"}

1. 업데이트 **[!UICONTROL Refunds Totals]** 다음과 같은 섹션:

   - 대상 **[!UICONTROL Refund Shipping]**&#x200B;배송비에서 환불할 금액을 입력합니다.

     이 필드에는 처음에 환불 가능한 주문의 총 배송 금액이 표시됩니다. 이미 환불된 배송금액을 제외하고 주문에서 전체 배송금액과 같습니다. 양과 마찬가지로 양을 줄일 수는 있지만 늘릴 수는 없습니다.

   - 대상 **[!UICONTROL Adjustment Refund]**&#x200B;를 누르고 주문(운송, 품목 또는 세금)의 특정 부분에 적용되지 않는 추가 환급으로 총 환급 금액에 합산할 값을 입력합니다. 또한 관리자가 비가상부담금 결제수단을 우선 환급하고자 할 때 상품권 등 가상화폐로 부분 환급하는 데도 활용할 수 있다.

     입력한 금액은 총 환불을 지급된 금액보다 높게 올릴 수 없습니다.

   - 대상 **[!UICONTROL Adjustment Fee]**&#x200B;을(를) 반환한 총액에서 뺄 값을 입력합니다.

     이 금액은 배송, 품목 또는 세금 등 주문의 특정 섹션에서 공제되지 않습니다.

1. 댓글을 추가하려면 **[!UICONTROL Credit Memo Comments]** 상자.

   - 고객에게 이메일 알림을 보내려면 **[!UICONTROL Email Copy of Credit Memo]** 확인란.

1. 클릭 **[!UICONTROL Update Totals]**.
...
1. 해당되는 경우 다음을 수행합니다.
   ...
   ![주문 총 환불](./assets/credit-memo-order-total-refund.png){width="600" zoomable="yes"}

### 수표 또는 금전 주문에 대한 오프라인 환불 발행

1. 다음에서 _관리자_ 사이드바, 이동 **[!UICONTROL Sales]** > **[!UICONTROL Orders]**.

1. 그리드에서 완료된 순서를 찾아 다음을 클릭하여 엽니다. **[!UICONTROL View]** 링크를 클릭합니다.

1. 페이지 상단의 단추 모음에서 를 클릭합니다. **[!UICONTROL Invoice]**.

1. 페이지 아래쪽으로 스크롤하여 를 클릭합니다. **[!UICONTROL Submit Invoice]**.
...
1. 에서 정보를 확인합니다. **[!UICONTROL Items to Refund]** 섹션을 참조하고 해당하는 경우 다음을 수행합니다.
   ...
   - 다음 항목 선택 **[!UICONTROL Return to Stock]** 반품된 제품을 재고로 반품하려는 경우 확인란입니다.

     Inventory management이 활성화된 경우, 재고 수량이 납품을 보낸 출처로 반환됩니다. 다음과 같은 경우 제품이 자동으로 재고로 반환됩니다. [제품 스톡 옵션](../inventory-management/enable.md) 이(가) (으)로 설정됨 `Automatically Return Credit Memo Item to Stock`.

   - 업데이트 **[!UICONTROL Qty to Refund]** 및 클릭 **[!UICONTROL Update Qty's]**.

     차감 금액은 환불 가능한 최대 금액을 초과할 수 없습니다.

1. 업데이트 **[!UICONTROL Refunds Totals]** 섹션에 해당하는 섹션을 추가했습니다.
   ...
   - 구매가 스토어 크레딧으로 지불된 경우 다음을 선택합니다. **[!UICONTROL Refund to Store Credit]** 고객 계정 잔액에 금액을 차감하는 체크박스.

1. 댓글을 추가하려면 **[!UICONTROL Credit Memo Comments]** 상자를 표시하고 다음을 수행합니다.

   - 고객에게 이메일 알림을 보내려면 **[!UICONTROL Email Copy of Credit Memo]** 확인란.

   - 이메일에 입력한 설명을 포함하려면 **[!UICONTROL Append Comments]** 확인란.

     대변 메모 통지의 상태는 대변 메모 번호 옆에 완료된 대변 메모에 나타납니다.

     ![환불 합계](./assets/credit-memo-order-totals.png){width="600" zoomable="yes"}

1. 프로세스를 완료하고 환불을 하려면 다음을 클릭하십시오. **[!UICONTROL Refund Offline]**.
...
| 필드 | 설명 |
|--- |--- |
| [!UICONTROL Order Number] | 주문 번호가 다음에 나타납니다. _주문 및 계정 정보_&#x200B;확인 이메일이 전송되었는지 보여 주는 메모가 뒤에 옵니다. |
| [!UICONTROL Order Date] | 주문이 이루어진 날짜와 시간. |
| [!UICONTROL Order Status] | 주문 상태를 다음으로 표시: `Complete`. |
...
완료된 대변 메모를 인쇄하거나 보려면 PDF 판독기가 설치되어 있어야 합니다. 다음을 다운로드할 수 있습니다. [Adobe Reader][1] 무료로.

1. 다음에서 _관리자_ 사이드바, 이동 **[!UICONTROL Sales]** > _[!UICONTROL Operations]_>**[!UICONTROL Credit Memos]**.
...
1. 설정 **[!UICONTROL Actions]** 제어 대상 `PDF Credit Memos` 및 클릭 **[!UICONTROL Submit]**.

   ![선택한 대변 메모 인쇄](./assets/credit-memos-print.png){width="600" zoomable="yes"}

1. 메시지가 표시되면 다음 중 하나를 수행합니다.

   - 문서를 저장하려면 를 클릭합니다. **[!UICONTROL Save]** 프롬프트에 따라 파일을 컴퓨터에 저장합니다. 다운로드가 완료되면 Adobe Reader에서 PDF을 열고 문서를 인쇄합니다.

   - 문서를 보려면 **[!UICONTROL Open]**. 인쇄용 PDF 대변 메모가 Adobe Reader에서 열립니다. 여기에서 대변 메모를 인쇄하거나 컴퓨터에 저장할 수 있습니다.
