...
페이지 레이아웃과 블록의 배치를 제어하는 코드는 XML로 작성됩니다 [위젯](widgets.md). 이러한 위젯을 사용하면 코드를 작성하지 않고도 페이지의 특정 위치 및 특정 제품이나 카테고리에 블록을 쉽게 배치할 수 있습니다. 가능한 모든 조합을 기억하기보다는 목록에서 각 옵션을 선택할 수 있습니다.

다음 목록은 일반적으로 블록이 배치되는 페이지 유형별 위치를 설명합니다. 페이지의 영역을 정의하는 방법에 대한 자세한 내용은 [표준 페이지 레이아웃](page-layout.md#standard-page-layouts).
...
| [!UICONTROL My Cart Extra Actions] | 콘텐츠는 아래에 표시됩니다. _장바구니 소계_ 고객이 페이지 상단에 있는 장바구니 아이콘을 클릭할 때. |
...
| [!UICONTROL Bottom Block Options Wrapper] | 사용자 지정 옵션이 추가되면 콘텐츠가 _장바구니에 추가_ 단추를 클릭합니다. |
...
| [!UICONTROL Page Header] | 콘텐츠는 페이지 헤더 아래에 나타납니다. |
| [!UICONTROL Page Top] | 페이지 상단에 콘텐츠가 표시됩니다. |
| [!UICONTROL PayPal Express Checkout (Payflow Edition) Shortcut Wrapper] | PayPal 결제 방법을 사용하는 경우 콘텐츠는 _PayPal 구매_ 단추를 클릭합니다. |
| [!UICONTROL PayPal Express Checkout Shortcut Wrapper] | PayPal 결제 방법을 사용하는 경우 콘텐츠는 _PayPal 구매_ 단추를 클릭합니다. |
...
| [!UICONTROL Tags List Before] | 컨텐츠가 위에 나타남 _[!UICONTROL Add Your Tags]_필드. |
