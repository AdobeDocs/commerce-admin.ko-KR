...
| [!UICONTROL Page Style] | 스토어 뷰 | PayPal 판매자 페이지의 모양을 결정합니다. 허용되는 값: **`paypal`** - PayPal 페이지 스타일을 사용합니다. <br/>**`primary`**- 계정 프로필에서 &quot;기본&quot; 스타일로 식별한 페이지 스타일을 사용합니다.<br/>**`your_custom_value`** - 계정 프로필에 지정된 사용자 지정 결제 페이지 스타일을 사용합니다. |
...
| [!UICONTROL Layout] | 스토어 보기 | PayPal 스마트 단추를 세로 또는 가로로 표시할지를 결정합니다. 옵션: <br/>**`Vertical`**- &quot;게스트 체크아웃 사용&quot;을 선택했는지 여부에 관계없이 구매자가 PayPal에 로그인하거나 PayPal 계정을 만들어야 합니다.<br/>**`Horizontal`** - &quot;게스트 체크아웃 사용&quot;을 선택하면 PayPal 팝업 창에 **`Pay with Debit Card or Credit Card`** 단추가 표시됩니다. 그렇지 않으면 구매자가 PayPal에 로그인하거나 PayPal 계정을 만들어야 합니다. |
