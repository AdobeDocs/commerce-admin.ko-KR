...
description: Commerce 관리자의 [!UICONTROL Sales] > [!UICONTROL Payment Methods] 페이지에서 [!UICONTROL PayPal Express Checkout] 섹션의 구성 설정을 검토하십시오.
...
![PayPal Express 체크 아웃 필수 설정](./assets/paypal-express-required-settings.png)<!-- zoom -->

<!-- [PayPal Express Checkout Required Settings](../../stores-purchase/paypal-express-checkout.html) -->

| 필드 | [범위](../../getting-started/websites-stores-views.md#scope-settings) | 설명 |
|--- |--- |--- |
| [!UICONTROL Enable this Solution] | 웹 사이트 | 고객이 사용할 수 있는 결제 방법으로 [!DNL PayPal Express Checkout]을(를) 활성화합니다. 옵션: `Yes` / `No` |
...
| [!UICONTROL Sandbox Mode] | 웹 사이트 | 테스트 환경에서 PayPal Express Checkout을 실행하려면 샌드박스 API 자격 증명을 입력하고 `Yes`(으)로 설정하십시오. 옵션: `Yes` / `No` |
| [!UICONTROL API Uses Proxy] | 웹 사이트 | 시스템에서 프록시 서버를 사용하여 Commerce과 PayPal 시스템 간의 연결을 설정하는 경우 `Yes`(으)로 설정하십시오. 옵션: `Yes` / `No` |
...
| [!UICONTROL Home Page] | 웹 사이트 | 홈 페이지에서 [!DNL PayPal Credit] 배너의 위치와 크기를 결정합니다. 옵션: <br/>**표시** - 스토어의 홈 페이지에 [!DNL PayPal Credit] 배너를 표시합니다. 옵션: `Yes` / `No` <br/>**위치** - 홈 페이지에서 [!DNL PayPal Credit] 배너의 위치를 결정합니다. 옵션: 머리글(가운데)/사이드바(오른쪽) <br/>**크기** - 홈 페이지에서 [!DNL PayPal Credit] 배너의 크기를 결정합니다. 옵션: `190 x 100` / `234 x 60` / `300 x 50` / `468 x 60` / `728 x 90` /` 800 x 66` |
...
| [!UICONTROL Sort Order] | 스토어 뷰 | 체크아웃 중에 다른 결제 방법과 함께 나열할 때 PayPal Express Checkout의 순서를 결정하는 번호가 나타납니다. 목록의 맨 위에 `0`을(를) 입력하십시오. |
...
| [!UICONTROL Display on Product Details Page] | 스토어 뷰 | 제품 페이지에 &quot;PayPal로 체크아웃&quot; 단추가 표시되는지 여부를 결정합니다. 옵션: `Yes` / `No` |
...
| [!UICONTROL Debug Mode] | 웹 사이트 | 스토어와 결제 시스템 간에 전송된 메시지를 로그 파일에 기록합니다. 옵션: `Yes` / `No` <br/><br/>**_Note:_** 로그 파일은 서버에 저장되며 개발자만 액세스할 수 있습니다. PCI 데이터 보안 표준에 따라 신용 카드 정보는 로그 파일에 기록되지 않습니다. |
...
| [!UICONTROL Payment Action] | 웹 사이트 | PayPal이 거래를 관리하는 방법을 결정합니다. 옵션: <br/>**권한 부여** - 구매를 승인하지만 자금을 보류합니다. 상인에게 &#39;포착&#39;되기 전까지는 그 금액이 인출되지 않는다. <br/>**판매** - 구매 금액이 승인되고 고객의 계정에서 즉시 인출됩니다. |
| [!UICONTROL Payment Applicable From] | 웹 사이트 | 적용 가능한 국가 선택의 범위를 결정합니다. 옵션: 허용된 모든 국가/특정 국가 |
| [!UICONTROL Countries Payment Applicable From] | 웹 사이트 | 지불이 수락되는 각 국가를 식별합니다. 선택한 국가에 청구 주소가 있는 고객만 이 결제 방법으로 구매할 수 있습니다. |
| [!UICONTROL Debug Mode] | 웹 사이트 | 로그 파일에 결제 시스템과의 통신을 기록합니다. 옵션: `Yes` / `No` <br/><br/>**_Note:_** 로그 파일은 서버에 저장되며 개발자만 액세스할 수 있습니다. PCI 데이터 보안 표준에 따라 신용 카드 정보는 로그 파일에 기록되지 않습니다. |
...
| [!UICONTROL Header Image URL] | 스토어 뷰 | 체크아웃 페이지의 왼쪽 위 모서리에 표시되는 이미지의 URL입니다. 최대 크기는 750 x 90픽셀입니다. <br/><br/>**_Note:_** PayPal에서는 이미지가 보안(https) 서버에 저장되어 있는 것이 좋습니다. 그렇지 않으면 고객의 브라우저가 &quot;페이지에 보안 및 비보안 항목이 모두 포함되어 있습니다.&quot;라고 경고할 수 있습니다. |
...
| [!UICONTROL Header Image Border Color] | 스토어 뷰 | 헤더 주위의 2픽셀 테두리에 대한 6자 [16진수](https://en.wikipedia.org/wiki/Web_colors) 코드입니다. |
| [!UICONTROL Page Background Color] | 스토어 뷰 | 머리글 및 결제 양식 뒤에 표시되는 체크아웃 페이지의 배경색에 대한 6자 [16진수](https://en.wikipedia.org/wiki/Web_colors) 코드입니다. |

{style="table-layout:auto"}

#### [!UICONTROL Customize Smart Buttons (Basic)]

![프론트엔드 환경 설정 - 스마트 단추 사용자 지정](./assets/payment-methods-paypal-payments-advanced-frontend-experience-settings2.png)<!-- zoom -->
...
| [!UICONTROL Label] | 스토어 보기 | PayPal이 스마트 결제 단추에 표시하는 텍스트입니다. 옵션: <br/>**`Checkout`**(&quot;PayPal Checkout&quot;으로 표시)<br/>**`Pay`**(&quot;PayPal로 표시) <br/>**`Buy Now`**(&quot;PayPal로 지금 구매&quot;로 표시)<br/>**`PayPal`**(&quot;PayPal&quot;로 표시) <br/>**`Installment`**(&quot;PayPal&quot;로 표시)<br/>**`Credit`**(&quot;PayPal CREDIT&quot;로 표시) |
| [!UICONTROL Layout] | 스토어 보기 | PayPal 스마트 단추를 세로 또는 가로로 표시할지를 결정합니다. 옵션: <br/>**`Vertical`**- &quot;게스트 체크아웃 사용&quot;을 선택했는지 여부에 관계없이 구매자가 PayPal에 로그인하거나 PayPal 계정을 만들어야 합니다.<br/>**`Horizontal`** - &quot;게스트 체크아웃 활성화&quot;를 선택하면 PayPal 팝업 창에 **`Pay with Debit Card or Credit Card`** 버튼이 표시됩니다. 그렇지 않으면 구매자가 PayPal에 로그인하거나 PayPal 계정을 만들어야 합니다. |
| [!UICONTROL Size] | 스토어 보기 | 스마트 결제 단추 크기를 설정합니다. 옵션: <br/>**`Medium`**- 250픽셀 x 35픽셀<br/>**`Large`** - 350픽셀 x 40픽셀 <br/>**`Responsive`**- (기본값) 컨테이너 너비에 맞게 조정됩니다. 최소 너비는 100픽셀이고 최대 너비는 500픽셀입니다. 높이는 너비에 따라 동적으로 조정됩니다. |
...
![프론트엔드 환경 설정 - 스마트 단추(기능) 사용자 지정](./assets/payment-methods-paypal-payments-advanced-frontend-experience-settings3.png)<!-- zoom -->
