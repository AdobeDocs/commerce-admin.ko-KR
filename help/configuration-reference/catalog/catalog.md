...
![상점 첫 화면](./assets/catalog-storefront.png)<!-- zoom -->

<!-- [Storefront](https://docs.magento.com/user-guide/catalog/navigation-product-listings.html) -->

| 필드 | [범위](../../getting-started/websites-stores-views.md#scope-settings) | 설명 |
|--- |--- |--- |
| [!UICONTROL List Mode] | 스토어 뷰 | 검색 결과 목록의 형식을 결정합니다. 옵션: <br/>**`Grid Only`**- 행 및 열 그리드로 목록 형식을 지정합니다. 각 제품은 그리드의 단일 셀에 나타납니다.<br/>**`List Only`** - 각 제품이 별도의 행에 있는 목록의 서식을 지정합니다. <br/>**`Grid (default / List)`**- 기본적으로 제품은 그리드 보기에 표시되며 목록 보기로 전환할 수 있습니다.<br/>**`List (default / Grid)`** - 기본적으로 제품은 목록 보기에 표시되며 그리드 보기로 전환할 수 있습니다. |
...
![제품 경고](./assets/catalog-product-alerts.png)<!-- zoom -->
...
![최근에 본/비교한 제품](./assets/catalog-recently-viewed-and-compared-products.png)<!-- zoom -->
...
| [!UICONTROL Autostart base video] | 스토어 뷰 | 페이지가 로드된 후 비디오를 자동으로 시작하려면 를 로 설정합니다. `Yes`. |
...
>[!NOTE]
>
>이 섹션에 설명된 표준 검색 구성은 [라이브 검색](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/overview.html).
...
![계층화된 탐색 - 자동(가격 범위 균등화)](./assets/layered-navigation-equalize-price-range.png)<!-- zoom -->

![계층화된 탐색 - 자동(제품 수 균등화)](./assets/layered-navigation-equalize-product-counts.png)<!-- zoom -->
...
| [!UICONTROL Price Navigation Step Calculation] | 스토어 뷰 | 다음을 결정하는 데 사용되는 방법을 결정합니다. [가격 탐색 단계](../../catalog/navigation-layered.md#configure-price-navigation)). 옵션: <br/>`Automatic (equalize price ranges)` - 그룹 내 제품의 가격 범위를 기준으로 계산합니다. <br/>`Automatic (equalize product counts)` - 그룹 내 제품 수를 기준으로 계산합니다. 그룹의 최소 제품 수에 대한 임계값을 설정하여 더 작은 그룹으로 나누어지지 않도록 합니다. <br/>`Manual` - 가격 간격에 입력한 분할 한도를 사용합니다. |
...
| [!UICONTROL Allow Browsing Category] | 웹 사이트 | 범주 탐색을 허용할 사용자를 결정합니다. 옵션: <br/>`Yes, for Everyone` - 모든 방문자와 고객이 카테고리를 검색할 수 있도록 허용합니다. <br/>`Yes, for Specified Customer Groups` - 선택한 고객 그룹의 구성원만 범주를 찾아볼 수 있습니다. <br/>`No, Redirect to Landing Page` - 카테고리에 대한 액세스를 거부하고 선택한 페이지로 리디렉션합니다. |
| [!UICONTROL Display Product Prices] | 웹 사이트 | 범주에 대한 제품 가격 표시를 제어합니다. 옵션: <br/>`Yes, for Everyone` - 모든 사용자가 범주 내의 제품 가격을 볼 수 있습니다. <br/>`Yes, for Specified Customer Groups` - 선택한 고객 그룹의 구성원만 범주에 있는 제품 가격을 볼 수 있습니다. <br/>`No` - 범주에 대한 제품 가격 표시를 끕니다. |
...
카탈로그 검색 구성에는 다음의 두 가지 변형이 있습니다. [[!DNL Live Search]](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/overview.html) 가 설치되어 있으며 기본 Adobe Commerce에서 사용할 수 있는 버전입니다. 설치에 대한 지침을 따르십시오.

### Adobe Commerce 포함 [!DNL Live Search]

라이브 검색이 설치되면 카탈로그 검색에 다음 구성 설정이 포함됩니다.

![라이브 검색에 대한 카탈로그 검색](./assets/catalog-search-live-search.png)<!-- zoom -->

<!-- [Catalog Search for Live Search](https://docs.magento.com/user-guide/catalog/search-configuration.html) -->

| 필드 | [범위](../../getting-started/websites-stores-views.md#scope-settings) | 설명 |
|--- |--- |--- |
| [!UICONTROL Minimal Query Length] | 스토어 뷰 | 카탈로그 검색에 허용되는 최소 문자 수입니다. 이 옵션에 대해 설정된 값은 Elasticsearch 검색 엔진 구성에 있는 해당 범위 세트와 호환되어야 합니다. 예를 들어 이 값을 로 설정하면 `2` Adobe Commerce에서 검색 엔진의 값을 업데이트합니다. |
...
Elasticsearch이 있는 기본 Adobe Commerce에는 다음 구성 설정이 포함되어 있습니다.

![카탈로그 검색 - Elasticsearch](./assets/catalog-search-elasticsearch.png)<!-- zoom -->
...
| [!UICONTROL Maximum Query Length] | 스토어 뷰 | 카탈로그 검색에 허용되는 최대 문자 수. 이 옵션에 대해 설정된 값은 Elasticsearch 검색 엔진 구성에 있는 해당 범위 세트와 호환되어야 합니다. 예를 들어, Adobe Commerce에서 이 값을 300으로 설정하면 검색 엔진에서 값을 업데이트합니다. |
| [!UICONTROL Number of top search results to cache] | 스토어 뷰 | 더 빠른 응답을 위해 캐시할 인기 검색어 및 결과 수. 값 입력 `0` 을(를) 두 번째로 입력하면 모든 검색어와 결과를 캐시합니다. 기본값: `100` |
| [!UICONTROL Enable EAV Indexer] | 글로벌 | 제품 EAV 인덱서를 사용할지 여부를 결정합니다. 이 기능은 색인 생성 속도를 향상시키고 타사 확장에서 색인을 사용하지 못하도록 제한합니다. 이 옵션은 Elasticsearch 또는 Elasticsearch 5.0 이상의 검색 엔진에만 표시됩니다. 기본 옵션: `Yes` 활성화됨 |
| [!UICONTROL Autocomplete Limit] | 스토어 뷰 | 검색 자동 완성을 위해 검색 필드 아래에 표시할 최대 검색 쿼리 수입니다. 이 양을 제한하면 검색 성능이 향상되고 표시된 목록 크기가 줄어듭니다. 기본값: `8` |
| 검색 엔진 | 글로벌 | 카탈로그 데이터 요청을 처리하는 데 필요한 검색 엔진을 식별합니다. Elasticsearch 7.6.x는 모든 Adobe Commerce 설치에 필요합니다. 옵션: `Elasticsearch 7` |
| [!UICONTROL Elasticsearch Server Hostname] | 글로벌 | Elasticsearch 서버의 이름을 지정합니다. 기본값: `elasticsearch.internal` |
| [!UICONTROL Elasticsearch Server Port] | 글로벌 | Elasticsearch에서 사용하는 서버 포트의 번호를 지정합니다. 기본값: `9200` |
| [!UICONTROL Elasticsearch Index Prefix] | 글로벌 | Elasticsearch 인덱스를 식별하는 접두사를 지정합니다. 기본값: `magento2` |
| [!UICONTROL Enable Elasticsearch HTTP Auth] | 글로벌 | 활성화된 경우 은 HTTP 인증을 사용하여 Elasticsearch 서버에 액세스하기 전에 사용자 이름과 암호를 묻습니다. 옵션: `Yes` / `No` |
| [!UICONTROL Elasticsearch HTTP Username] | 글로벌 | 날짜 _Elasticsearch HTTP 인증 활성화_ 이(가) (으)로 설정됨 `Yes`Elasticsearch HTTP 인증을 위한 사용자 이름을 지정합니다. |
| [!UICONTROL Elasticsearch HTTP Password] | 글로벌 | 날짜 _Elasticsearch HTTP 인증 활성화_ 이(가) (으)로 설정됨 `Yes`Elasticsearch HTTP 인증의 암호를 지정합니다. |
| [!UICONTROL Elasticsearch Server Timeout] | 글로벌 | 서버 제한 시간(초)을 결정합니다. 기본값: `15` |
| [!UICONTROL Test Connection] |  | Elasticsearch 연결을 확인합니다. |
| [!UICONTROL Enable Search Recommendations] | 스토어 뷰 | 검색 결과가 반환되지 않고 아래에 표시될 때 검색 권장 사항이 제공되는지 여부를 결정합니다. `Related search terms` 섹션에 있는 마지막 항목이 될 필요가 없습니다. 옵션: `Yes` / `No` <br/>예로 설정된 경우 다음에 대한 추가 옵션이 표시됩니다. _[!UICONTROL Search Recommendations Count]_및_[!UICONTROL Shows Results Count for Each Recommendation]_. |
...
| [!UICONTROL Shareable] | 웹 사이트 | 고객이 다운로드 링크에 액세스하기 위해 계정에 로그인해야 하는지 여부를 결정합니다. 옵션: <br/>**예** - 이메일을 통해 링크를 보낸 후 다른 사용자와 공유할 수 있습니다. <br/>**아니요** - 고객이 계정에 로그인하여 다운로드 링크에 액세스해야 합니다. |
...
| 필드 | [범위](../../getting-started/websites-stores-views.md#scope-settings) | 설명 |
|--- |--- |--- |
| [!UICONTROL Use JavaScript Calendar] | 스토어 뷰 | JavaScript 달력이 날짜 필드에 대한 입력 컨트롤로 사용되는지 여부를 결정합니다. 옵션: `Yes` / `No` <br/>로 설정된 경우 `No`, 날짜 필드의 각 부분에 대해 별도의 드롭다운이 표시됩니다. |
...
| 필드 | [범위](../../getting-started/websites-stores-views.md#scope-settings) | 설명 |
|--- |--- |--- |
| [!UICONTROL Maximum Number of Products in Related Products List] | 글로벌 | 에 나타날 수 있는 최대 제품 수를 결정합니다. _관련 제품_ 목록을 표시합니다. |
...
| [!UICONTROL Rotation Mode for Products in Related Products List] | 글로벌 | 에서 제품의 순서를 결정합니다. _관련 제품_ 목록이 나타납니다. 옵션: `Do not rotate` / `Shuffle` |
...
| [!UICONTROL Maximum Number of Products in Upsell Product List] | 글로벌 | 에 나타날 수 있는 최대 제품 수를 결정합니다. _상향 판매 제품_ 목록을 표시합니다. |
