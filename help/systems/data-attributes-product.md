...
| `product_type` | 제품 유형을 나타냅니다. 값:<br/>`simple` — 일반적으로 단일 단위 또는 고정 수량으로 판매되는 유형 항목.<br/>`grouped` — 세트로 판매되는 별도의 제품 그룹입니다.<br/>`configurable` — 구매하기 전에 고객이 선택해야 하는 여러 옵션이 있는 제품입니다. 개별 SKU를 사용하는 개별 제품을 나타내므로 각 변형 세트에 대해 인벤토리를 관리할 수 있습니다. 예를 들어 구성 가능한 제품의 색상 및 크기 조합은 카탈로그의 특정 SKU와 연결됩니다.<br/>`virtual` — 배송이 필요하지 않고 재고에 보관되지 않는 유형의 제품이 아닙니다. 서비스, 멤버십, 구독 등을 예로 들 수 있습니다.<br/>`bundle` — 함께 판매되는 간단한 제품들로 구성된 사용자 정의 가능한 제품 세트입니다. |
...
| product_online | 제품을 스토어에서 판매할 수 있는지 여부를 결정합니다. 값: <br/>`1` — (예) 제품이 활성화되어 있으며 판매 가능합니다.<br/>`2` — (아니요) 제품이 비활성화되어 판매에 사용할 수 없습니다. |
| `tax_class_name` | 이 제품과 연계된 세금 분류의 이름. |
| `visibility` | 제품이 카탈로그에 표시되는지, 그리고 검색할 수 있게 되는지를 결정합니다. 값:<br/>`Not Visible Individually` — 다른 제품의 변형으로 사용할 수 있지만 해당 제품은 제품 목록에 포함되지 않습니다.<br/>`Catalog` — 제품이 모든 카탈로그 목록에 표시됩니다.<br/>`Search` —제품을 검색 작업에 사용할 수 있습니다.<br/>`Catalog, Search` — 제품이 카탈로그 목록에 포함되어 있으며 검색할 수도 있습니다. |
...
| `map_enabled` | 구성에서 최소 광고 가격 이 활성화되어 있는지 여부를 결정합니다. 값: <br/>`1` — (예) MAP이 활성화되어 있습니다.<br/>`0` (또는 비어 있음) - (아니요) MAP이 활성화되지 않습니다. |
...
| `msrp_display_actual_price_type` | 제품의 실제 가격이 고객에게 표시되는 위치를 결정합니다. 값:<br/>`In Cart` — 장바구니의 실제 제품 가격을 표시합니다.<br/>`Before Order Confirmation` — 주문이 확정되기 바로 전 체크아웃 프로세스가 끝날 때 실제 제품 가격을 표시합니다.<br/>`On Gesture` — 고객이 _가격 클릭_ 또는 _무엇 때문에?_&#x200B;을 클릭하면 실제 제품 가격이 팝업으로 표시됩니다. 링크를 클릭합니다. |
...
| `use_config_min_qty` | 구성의 기본값이 사용되는지 여부를 확인하고 구성 설정 사용 확인란에 해당합니다. 값: <br/>`1` — (예) 이 특성의 값에 기본 구성 설정이 사용됩니다.<br/>`0` (또는 비어 있음) — (아니오) 이 속성 값에 대해 기본 구성이 재정의될 수 있습니다. |
...
| `bundle_values` | 번들 옵션과 연결된 각 값을 설명합니다. 예: `name=Bundle Option One,name_store_view_1=Group 1,type=dropdown; required=1,sku=sku-option2,price=10,default=0,default_qty=1.0000,price_type=fixed,can_change_qty=0,price_website_website_1=30.000000,price_type_website_website_1=fixed`<br/><br/>`name_[store-view-code]` 옵션에서 `[store-view-code]`은(는) 스토어 보기의 이름입니다.<br/></br>`price_website_[website-code]` 및 `price_type_website_[website-code]` 옵션은 웹 사이트 `[website-code]`의 제품 가격을 지정합니다. [카탈로그 가격 범위](../catalog/catalog-price-scope.md)가 *글로벌*(으)로 설정된 경우 이러한 옵션을 사용할 수 없습니다. |
