...
>[!WARNING]
>
>Adobe Commerce [지원 사례](https://experienceleague.adobe.com/ko/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide#support-case)를 제출할 때 Commerce 조직을 찾을 수 없는 경우 일반적으로 다음 중 하나를 의미합니다. 계정 소유자가 Adobe ID을 만들지 않았거나 Adobe ID이 있지만 Commerce 계정에 연결되어 있지 않습니다.
...
| 시나리오 | 진행 상황 | Adobe ID으로 수행할 작업 | MAGE ID / Commerce 계정에 대해 수행할 작업 | 결과 |
| --- | --- | --- | --- | --- |
| Adobe 및 Commerce의 새로운 기능 | Adobe ID 없음, 이미지 ID 없음 | https://account.magento.com에 로그인하는 동안 메시지가 표시되면 Adobe ID을 만듭니다. | Adobe ID이 만들어지면 https://account.magento.com에 다시 로그인합니다. | Commerce 계정이 만들어지고 MAGE ID가 생성되어 Adobe ID에 연결됩니다 |
| Adobe ID이 있지만 아직 MAGE ID가 없음 | 다른 Adobe 제품만 사용 | https://account.magento.com에서 Adobe ID으로 로그인 | 첫 번째 로그인이 성공하면 Commerce 계정 및 MAGE ID가 만들어집니다 | MAGE ID가 자동으로 생성되고 연결됩니다. |
| 이전 MAGE ID를 가진 이전 Commerce 고객 | 기존 MAGE ID는 있지만 Adobe ID은 없음 | 기존 MAGE ID와 동일한 이메일을 사용하여 https://account.adobe.com에서 Adobe ID을 만듭니다 | &quot;Adobe ID으로 로그인&quot;을 사용하여 https://account.magento.com에 로그인합니다. | 기존 MAGE ID가 새 Adobe ID에 연결됨 |
| Adobe ID 및 MAGE ID가 존재하지만 연결되어 있지 않음 | Adobe ID 및 Commerce에 대한 이메일은 서로 다릅니다 | Commerce 계정 이메일과 일치하도록 Adobe ID 이메일 정렬(또는 소유권에 따라 그 반대) | 이메일이 일치하면 https://account.magento.com에서 &quot;Adobe ID으로 로그인&quot;을 통해 로그인합니다. | Adobe ID이 로그인이 되고, MAGE ID는 자격 식별자로 유지됩니다 |
| Adobe ID이 있지만 &quot;MAGE ID 없음&quot; | https://account.magento.com에 로그인하지 않음 | 기존 Adobe ID 사용 | 처음 https://account.magento.com에 로그인합니다. | 이 첫 번째 로그인은 MAGE ID를 생성하고 연결합니다. |
| 클라우드 프로젝트 로그인만 사용(accounts.magento.cloud) | 클라우드 프로젝트에 액세스할 수 있지만 클래식 Commerce 계정에는 액세스할 수 없음 | 클라우드 프로젝트용 Adobe ID을 계속 사용 | 마켓플레이스/라이센스가 필요한 경우 동일한 Adobe ID으로 https://account.magento.com에 로그인하십시오 | 클래식 Commerce 계정(MAGE ID 포함)이 생성되고 연결됩니다 |

사용자가 Commerce 권한이 있어야 하지만 MAGE ID가 표시되지 않는다고 생각되면 기본 다음 단계는 Commerce 계정과 MAGE ID를 만들거나 올바르게 연결할 수 있도록 Adobe ID을 사용하여 https://account.magento.com에 로그인하는 것입니다.
