...
또한 [관리 기본 URL 구성을 업데이트](/help/configuration-reference/advanced/admin.md#admin-base-url)하여 기본 `/admin` 끝점을 사용자 지정 경로로 변경하는 것이 좋습니다. 사용자 지정 경로를 구성하면 다음과 같은 보안 이점이 있습니다.
...
>[!NOTE]
>
>위반이 의심되는 경우 알 수 없는 모든 관리자 사용자를 제거하고 모든 관리자 암호를 재설정하고 [보안 작업 계획](/help/systems/security.md)을 검토하여 추가 단계를 검토하십시오.
