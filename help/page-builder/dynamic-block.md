---
title: 콘텐츠 추가 - 동적 블록
description: 재사용 가능한 동적 블록을 추가하는 데 사용되는 동적 블록 콘텐츠 유형에 대해 알아봅니다. [!DNL Page Builder] 스테이지.
...
동적 블록 콘텐츠 유형을 사용하여 기존 항목 추가 [동적 블록](../content-design/dynamic-blocks.md) (으)로 [[!DNL Page Builder] 단계](workspace.md#stage).
...
| 설정 | ![설정 아이콘](./assets/pb-icon-settings.png){width="25"} | 다음을 엽니다. _블록 편집_ 블록을 선택하고 컨테이너의 속성을 변경할 수 있는 페이지입니다. |
...
1. 다음 위치로 이동 [!DNL Page Builder] 대상 페이지, 블록, 제품 또는 카테고리의 작업 영역.

1. 다음에서 [!DNL Page Builder] 패널, 확장 **[!UICONTROL Add Content]** 드래그 **[!UICONTROL Dynamic Block]** 자리 표시자가 스테이지에 표시됩니다.

   ![동적 블록 자리 표시자를 스테이지로 드래그](./assets/pb-dynamic-block-drag.png){width="600" zoomable="yes"}

1. 빈 동적 블록 컨테이너에 마우스를 가져다 대고 도구 상자를 표시하고 _설정_ ( ![설정 아이콘](./assets/pb-icon-settings.png){width="20"} ) 아이콘.

   ![동적 블록 도구 상자](./assets/pb-dynamic-block-settings.png){width="600" zoomable="yes"}

1. 다음에서 _동적 블록 편집_ 페이지, 클릭 **[!UICONTROL Select Dynamic Block]** 목록을 사용하여 블록을 선택합니다.

   ![동적 블록 선택](./assets/pb-dynamic-block-select.png){width="600" zoomable="yes"}

   목록에서 삽입할 동적 블록을 찾은 다음 **[!UICONTROL Select]**. 그런 다음 을 클릭합니다. **[!UICONTROL Add Selected]**.
...
1. 상위 컨테이너 내에서 동적 블록의 위치를 제어하려면 **[!UICONTROL Alignment]**:
...
1. 설정 **[!UICONTROL Border]** 동적 블록 컨테이너의 네 면에 모두 적용되는 스타일:
...
1. 다음에 대한 값을 픽셀 단위로 입력하십시오. **[!UICONTROL Margins and Padding]** 동적 블록 컨테이너의 외부 여백 및 내부 패딩을 결정할 수 있습니다.
...
1. 필요한 경우 동적 블록을 변경합니다.
   ...
   - 클릭 **[!UICONTROL Select Dynamic Block]**.

     ![다른 동적 블록 선택](./assets/pb-add-content-dynamic-block-select.png){width="20"}

   - 활성 동적 블록 목록에서 **[!UICONTROL Select]** 추가할 블록에 사용됩니다.
...
1. 동적 블록 컨테이너에 마우스를 가져다 대고 도구 상자를 표시한 다음 _복제_ ( ![중복 아이콘](./assets/pb-icon-duplicate.png){width="20"} ) 아이콘.

   복제본이 원본 바로 아래에 나타납니다.

   ![동적 블록 복제](./assets/pb-add-content-dynamic-block-duplicate.png){width="500" zoomable="yes"}

1. 새 동적 블록을 다른 위치로 이동하려면 해당 컨테이너를 마우스로 가리킨 다음 을 선택합니다 _이동_ ( ![이동 아이콘](./assets/pb-icon-move.png){width="20"} )을 클릭하여 제품에서 사용할 수 있습니다.
...
1. 동적 블록 컨테이너에 마우스를 가져다 대고 도구 상자를 표시한 다음 _제거_ ( ![제거 아이콘](./assets/pb-icon-remove.png){width="20"} ) 아이콘.
