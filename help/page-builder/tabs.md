---
title: 레이아웃 - 탭
description: 에서 탭 집합을 추가하는 데 사용되는 탭 콘텐츠 유형에 대해 알아봅니다. [!DNL Page Builder] 스테이지.
...
사용 _탭_ 에 탭 집합을 추가하는 콘텐츠 유형 [[!DNL Page Builder] 단계](workspace.md#stage). 패널에서 단계로 탭 자리 표시자를 드래그하면 처음에는 단일 기본 탭이 나타납니다. 탭을 더 추가하여 전체 세트를 만들 수 있습니다. 탭 세트의 너비는 상위 컨테이너의 너비와 패딩 설정에 의해 결정됩니다.

![탭 세트](./assets/pb-layout-tab-example.png){width="500" zoomable="yes"}

{{$include /help/_includes/page-builder-save-timeout.md}}

## 툴박스

을 사용하여 작업할 때 _탭_ 콘텐츠 유형에서는 개별 탭과 하나 이상의 탭이 있는 탭 컨테이너를 추가하고 편집합니다. 각 탭에는 탭에서 탭을 디자인하는 데 사용하는 도구 상자가 있습니다 [!DNL Page Builder] 스테이지.

### 개별 탭 도구 상자

![탭 도구 상자](./assets/pb-layout-tab1-toolbox.png){width="500" zoomable="yes"}
...
![탭 컨테이너 도구 상자](./assets/pb-tabs-toolbox-settings.png){width="500" zoomable="yes"}
...
1. 다음에서 [!DNL Page Builder] 패널 아래 _[!UICONTROL Layout]_를 클릭하고&#x200B;**[!UICONTROL Tabs]**스테이지로 직접 이동하거나 스테이지의 행 또는 열에 대한 자리 표시자

   ![행으로 탭 드래그](./assets/pb-layout-tabs-drag-row.png){width="600" zoomable="yes"}

1. 다음을 클릭합니다. **[!UICONTROL Tab 1]** 레이블을 지정하여 개별 탭 도구 상자를 표시하고 _설정_ ( ![설정 아이콘](./assets/pb-icon-settings.png){width="20"} ) 아이콘.

1. 다음을 입력합니다. **[!UICONTROL Tab Name]** 레이블로 사용하려는 경우입니다.

   ![탭 이름 입력](./assets/pb-layout-tab1-toolbox-settings-general-tab-name.png){width="600" zoomable="yes"}

1. 필요한 경우 **[!UICONTROL Minimum Height]** 탭.

   이 값은 유효한 CSS 단위가 있는 숫자일 수 있습니다(예: `100px`, `50%`, `50em`, `100vh`) 또는 계산(예: `100vh - 237px`).

1. 선택 **[!UICONTROL Vertical Alignment]** 을 설정하여 탭에 추가된 모든 콘텐츠 컨테이너(위쪽, 가운데 또는 아래쪽)를 정렬합니다.
...
1. 탭 컨테이너 위로 마우스를 가져가면 도구 상자를 표시하고 _추가_ ( ![추가 아이콘](./assets/pb-icon-add.png){width="20"} ) 아이콘.

1. 을(를) 클릭합니다. **[!UICONTROL Tab 2]** 레이블을 지정하여 커서를 표시하고 탭의 고유한 레이블을 입력합니다.

1. 스테이지에서 두 번째 탭을 다시 클릭하고 _복제_ ( ![중복 아이콘](./assets/pb-icon-duplicate.png){width="20"} ) 아이콘.

1. YourName **[!UICONTROL Copy]** 레이블을 지정하여 커서를 표시하고 세 번째 탭에 대한 사용자 고유의 레이블을 입력합니다.
...
1. 을(를) 선택하고 드래그합니다. _이동_ ( ![이동 아이콘](./assets/pb-icon-move.png){width="20"} ) 아이콘을 클릭합니다. 이 아이콘은 탭 레이블 텍스트 바로 앞에 나타나며 탭 집합 내의 새 위치에 표시됩니다.
...
1. 다음에서 [!DNL Page Builder] 패널, 확장 **[!UICONTROL Elements]** 드래그 **텍스트** 탭에 대한 자리 표시자

1. 편집기에 텍스트를 입력하거나 붙여 넣고 편집기 도구 모음을 사용하여 필요에 따라 서식을 지정합니다.

   다음을 참조하십시오 [요소 - 텍스트](text.md) 텍스트 콘텐츠 유형 작업에 대한 자세한 내용을 보려면
...
1. 개별 탭에 마우스를 가져다 대고 도구 상자를 표시하고 _설정_ ( ![설정 아이콘](./assets/pb-icon-settings.png){width="20"} ) 아이콘.

1. 필요한 경우 탭의 기본 설정을 변경합니다.

   - **[!UICONTROL Tab Name]** - 탭 레이블에 대해 수정된 텍스트를 입력합니다. 스테이지에서 직접 레이블을 수정할 수도 있습니다.

   - **[!UICONTROL Minimum Height]** - 자동 높이를 무시하려면 픽셀로 입력합니다. 예를 들어, 전체 이미지가 표시되도록 하기 위해 배경 이미지의 높이와 일치하도록 최소 높이를 설정할 수 있습니다.
...
- **[!UICONTROL Background Color]** - 색상 견본을 선택하거나, 색상 선택기를 클릭하거나, 유효한 색상 이름 또는 이에 해당하는 16진수 값을 입력하여 배경색을 지정합니다. 이 설정은 행의 배경색을 결정합니다. 색상의 불투명도를 조정할 수도 있습니다.
...
- **[!UICONTROL Background Image]** - 필요한 경우 제공된 도구를 사용하여 탭에 적용할 배경 이미지를 선택합니다.
...
- **[!UICONTROL Background Position]** - 탭을 기준으로 배경 이미지가 고정되는 방식을 선택합니다. `Top Left` / `Top Center` / `Top Right` / `Center Left` / `Center` / `Center Right` / `Bottom Left` / `Bottom Center` / `Bottom Right`

- **[!UICONTROL Background Attachment]** - 첨부 파일 유형을 선택하여 스크롤 페이지와 관련하여 배경 이미지가 이동하는 방식을 결정합니다.
...
- **[!UICONTROL Background Repeat]** - 다음으로 설정 `Yes` 를 클릭하여 배경 이미지를 반복하여 탭의 사용 가능한 공간을 채웁니다.

### 고급

- 탭에 추가되는 콘텐츠 컨테이너의 가로 맞춤을 제어하려면 **[!UICONTROL Alignment]** .
...
- 설정 **[!UICONTROL Border]** 탭 컨테이너의 네 면에 모두 적용되는 스타일:
...
1. 탭 집합 컨테이너의 위쪽 테두리를 마우스로 가리키면 도구 상자가 표시되고 _설정_ ( ![설정 아이콘](./assets/pb-icon-settings.png){width="20"} ) 아이콘.

1. 필요한 경우 **[!UICONTROL Default Active Tab]**.

   페이지가 로드될 때 활성화할 세트에서 탭을 선택합니다.

1. 다음을 입력합니다. **[!UICONTROL Minimum Height]**, 탭 세트의 자동 높이를 오버라이드하려면 픽셀 단위로 입력하십시오.

1. 탐색 탭을 탭 세트의 맨 위에 배치하려면 **[!UICONTROL Tab Navigation Alignment]** (`Left`, `Center`, 또는 `Right`).

   ![오른쪽에 정렬된 탐색 탭](./assets/pb-layout-tabs-navigation-alignment-right.png){width="500" zoomable="yes"}

1. 탭 세트에 대한 고급 옵션을 설정합니다.

   - 상위 컨테이너 내에 있는 탭 집합의 위치를 제어하려면 **[!UICONTROL Alignment]**:
   ...
   - 설정 **[!UICONTROL Border]** 탭 컨테이너의 네 면에 적용된 스타일:
   ...
   - (선택 사항) 다음 이름을 지정합니다 **[!UICONTROL CSS classes]** 탭 컨테이너에 적용할 현재 스타일 시트

     여러 클래스 이름은 공백으로 구분합니다.

   - 다음에 대한 값을 픽셀 단위로 입력하십시오. **[!UICONTROL Margins and Padding]** 을 클릭하여 탭 컨테이너의 외부 여백 및 내부 패딩을 결정합니다.
