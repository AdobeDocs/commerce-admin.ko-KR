---
title: 미디어 - 이미지
description: 에 JPG, GIF 또는 PNG 이미지를 추가하는 데 사용되는 이미지 콘텐츠 유형에 대해 알아봅니다 [!DNL Page Builder] 스테이지.
...
사용 _이미지_ JPG, GIF 또는 PNG 이미지를 추가할 컨텐츠 유형 [[!DNL Page Builder] 단계](workspace.md#stage). 기본 데스크톱 이미지 외에 모바일 장치용 2차 이미지를 지정할 수 있습니다. 이미지 아래에 나타나는 캡션을 추가하고 이미지를 URL, 제품, 카테고리 또는 페이지에 연결할 수도 있습니다.

>[!TIP]
>
>다음을 사용할 수 있습니다. [Adobe Stock 통합](../content-design/adobe-stock.md) 에서 제공한 수백만 개 중 적절한 자산을 찾아 저장 [Adobe Stock](https://stock.adobe.com). 다음을 참조하십시오 [Adobe Stock 이미지 사용](../content-design/adobe-stock-manage.md) Adobe Stock 에셋을 검색하고, 세분화하고, 갤러리에 저장하는 방법에 대한 자세한 내용을 보려면 여기를 클릭하십시오.
...
| 설정 | ![설정 아이콘](./assets/pb-icon-settings.png){width="25"} | 다음을 엽니다. _이미지 편집_ 이미지 및 컨테이너의 속성을 변경할 수 있는 페이지. |
...
1. 다음에서 [!DNL Page Builder] 패널, 확장 **[!UICONTROL Media]** 을(를) 끌어서 놓습니다. **[!UICONTROL Image]** 대상 컨테이너에 대한 자리 표시자

   행, 열 또는 탭에 이미지를 추가할 수 있습니다. 다음 예제에서는 이미지가 빈 열로 드래그됩니다.

   ![이미지 컨텐츠 유형을 스테이지로 드래그](./assets/pb-media-image-drag.png){width="600" zoomable="yes"}

1. 다음 방법 중 하나를 사용하여 이미지 에셋을 추가합니다.

   ![스테이지의 이미지 업로드 또는 갤러리에서 선택 도구](./assets/pb-media-image-upload-select.png){width="500" zoomable="yes"}

   >[!NOTE]
   >
   >최대 파일 크기는 4MB입니다. 지원되는 파일 유형은 JPG, GIF 및 PNG입니다.

   - _**새 이미지 업로드**_: 이 방법을 사용하여 시스템에서 새 이미지 파일을 업로드합니다.

      - 클릭 **[!UICONTROL Upload Image]**.

      - 갤러리와 대상 컨테이너에 추가할 이미지를 찾아 선택합니다.

     또는 시스템에서 이미지 파일을 드래그하여 _카메라_ ( ![카메라 아이콘](./assets/pb-icon-camera.png){width="20"} ) 아이콘.

   - _**기존 자산 선택**_: 이 방법을 사용하여 미디어 스토리지/갤러리에서 기존 이미지 에셋을 선택합니다.

      - 클릭 **[!UICONTROL Select from Gallery]**.
   ...
   - _**Adobe Stock 이미지 검색 및 선택**_: 이 메서드를 사용하여 Adobe Stock에서 이미지를 찾습니다.
     ...
     >[!NOTE]
     >
     >이 메서드는 [Adobe Stock 통합](../content-design/adobe-stock.md) 관리자용으로 구성되었습니다.

      - 클릭 **[!UICONTROL Search Adobe Stock]** 이미지를 검색합니다.

      - 미리 보기 또는 라이선스가 부여된 이미지를 갤러리에 저장합니다.

        다음을 참조하십시오 [Adobe Stock 이미지 사용](../content-design/adobe-stock-manage.md) Adobe Stock assets 작업에 대한 자세한 정보입니다.
   ...
   >[!NOTE]
   >
   >다음 [배너](banner.md) 및 [슬라이더](slider.md) 콘텐츠 유형에는 다음도 포함됩니다 _이미지 업로드_ 및 _갤러리에서 선택_ 이미지 추가 옵션.

   ![열의 이미지](./assets/pb-media-image-column1-giftcard.png){width="500" zoomable="yes"}

## 이미지 설정 변경

1. 이미지 컨테이너 위로 마우스를 가져가면 도구 상자가 표시되고 _설정_ (![설정 아이콘](./assets/pb-icon-settings.png){width="20"} ) 아이콘.
파일 이름, 치수 및 파일 크기가 현재 이미지 아래에 나타납니다.

   ![현재 이미지](./assets/pb-media-image-settings-image.png){width="600" zoomable="yes"}

1. 현재 항목 변경하기 **[!UICONTROL Image]**&#x200B;를 클릭하고 다음 중 하나를 수행합니다.
...
1. 을(를) 추가하려면 **[!UICONTROL Mobile Image]**&#x200B;를 설치한 후 이전 단계에서 설명한 것과 동일한 방법을 사용하여 모바일 장치에서 표시하는 데 사용할 이미지를 선택합니다.

   ![모바일 이미지](./assets/pb-media-image-settings-mobile-image.png){width="600" zoomable="yes"}

1. 필요한 경우 **[!UICONTROL Link]** 이미지.
...
1. 을(를) 추가하려면 **[!UICONTROL Image Caption]**&#x200B;이미지 아래에 표시할 텍스트를 입력합니다.

   캡션의 형식은 현재 테마와 연결된 스타일시트에 의해 결정됩니다.

   캡션은 일반적으로 이미지 아래에 표시되며, 방문자 및 검색 엔진을 위한 이미지에 대한 정보를 제공합니다. 사이트를 여러 언어로 사용할 수 있는 경우 동일한 이미지를 사용할 수 있지만 캡션을 번역할 수 있습니다. HTML에서 `<figcaption>` 태그는 `<figure>` 태그에 가깝게 배치하십시오. `<figcaption>This is the image caption</figcaption>`
...
1. 이미지 컨테이너 위로 마우스를 가져가 도구 상자를 표시하고 _이동_ (![이동 아이콘](./assets/pb-icon-move.png){width="20"} ) 아이콘.

   ![이미지 이동](./assets/pb-media-image-column1-move-giftcard.png){width="500" zoomable="yes"}

1. 이미지를 선택하여 빨간색 지침 바로 아래의 새 위치로 드래그합니다.

   ![빨간색 가이드라인을 사용하여 이미지 배치](./assets/pb-media-image-column2-move-giftcard-red-guideline.png){width="500" zoomable="yes"}

## 이미지 제거

1. 이미지 컨테이너 위로 마우스를 가져가 도구 상자를 표시하고 _제거_ ( ![제거 아이콘](./assets/pb-icon-remove.png){width="20"} ) 아이콘.
...
- 컨테이너에 추가된 이미지의 가로 위치를 제어하려면 **[!UICONTROL Alignment]**.
...
- 설정 **[!UICONTROL Border]** 이미지 컨테이너의 네 면 모두에 적용된 스타일:
...
- (선택 사항) 다음 이름을 지정합니다 **[!UICONTROL CSS classes]** 이미지 컨테이너에 적용할 현재 스타일시트에서 가져옵니다.

  여러 클래스 이름은 공백으로 구분합니다.

- 다음에 대한 값을 픽셀 단위로 입력하십시오. **[!UICONTROL Margins and Padding]** 이미지 컨테이너의 외부 여백 및 내부 패딩을 지정합니다.
