...
1. 고객 편집 페이지에서 **[!UICONTROL Generate Login Code]**&#x200B;을(를) 클릭합니다.

   ![고객 편집 페이지의 고객 로그인 OTC 가져오기 단추](assets/get-customer-login-otc-button-new.png){width="600" zoomable="yes"}

1. **[!UICONTROL Reason]**(필수)을(를) 입력하고 **[!UICONTROL Request]**&#x200B;을(를) 클릭합니다.

   이유 필드가 있는 ![OTC 요청 양식](assets/otc-reason-modal-new.png){width="600" zoomable="yes"}
...
1. 생성된 OTC가 모달에 표시됩니다. 고객 인증을 위해 이 코드를 `generateCustomerToken` 또는 `exchangeOtpForCustomerToken` GraphQL 돌연변이와 함께 사용하십시오.
   ...
   ![생성된 OTC가 모달에 표시됨](assets/otc-generated-code-new.png){width="300" zoomable="yes"}
...
1. 이전 섹션에 나열된 페이지에서 **[!UICONTROL Login as Customer]** 또는 **[!UICONTROL Generate Login Code]**&#x200B;을(를) 클릭합니다.
